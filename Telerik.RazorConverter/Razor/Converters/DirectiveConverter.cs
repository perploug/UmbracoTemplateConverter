    ...
    public class DirectiveConverter : INodeConverter<IRazorNode>
    {
        ...
        public IList<IRazorNode> ConvertNode(IWebFormsNode node)
        {
            ...
            if (directiveNode != null)
            {
                ...
                else if (directiveNode.Attributes.ContainsKey("MasterPageFile"))
                {
                    var view = "null";

                    if(!directiveNode.Attributes["MasterPageFile"].StartsWith("~/umbraco"))
                    {
                        view = "\"" +
                               directiveNode.Attributes["MasterPageFile"]
                                   .TrimStart('~')
                                   .ToLower()
                                   .Replace(".master", ".cshtml")
                                   .Replace("/masterpages/", "")
                               + "\"";
                    }

                    result.Add(new RazorTextNodeFactory().CreateTextNode("@inherits Umbraco.Web.Mvc.UmbracoTemplatePage\n@{\n\tLayout = " + view + ";\n}\n"));
                }
            }
